...
struct mm_struct *get_task_mm(struct task_struct *task)
	...
	if (NULL == mm_task) {
		printk(KERN_INFO "mm_task error\n");
		goto finish;
	}
	printk(KERN_INFO "get_task_mm :mm_task->mm_users :%d, mm_task->count :%d\n",
			mm_task->mm_users, mm_task->mm_count);
#if 0
void mmput(struct mm_struct *mm)
#endif
	mmput(mm_task);
	printk(KERN_INFO "mmput : mm_task->mm_users :%d, mm_task->count :%d\n",
			mm_task->mm_users, mm_task->mm_count);
finish:
	return 0;
}
