...
static int myfunc(void *arg)
{
	...
	printk(KERN_INFO "parent state :%ld befor wake_up_process\n",
...
int wake_up_process(struct task_struct *p)
	...
	printk(KERN_INFO "parent state :%ld after wake_up_process\n",
			current->real_parent->state);
#endif
	return 0;
}
...
static int __init interruptible_sleep_on_timeout_init(void)
{
	...
	wait_queue_t data;
	long remaintime;
	printk(KERN_INFO "interruptible_sleep_on_timeout_init\n");
#if 0
...
#endif
	remaintime = interruptible_sleep_on_timeout(&head, 100);
...
{
	printk(KERN_INFO "interruptible_sleep_on_timeout_exit\n");
}
