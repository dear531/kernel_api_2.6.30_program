...
#endif
	printk(KERN_INFO "befor page_cache_get pages->_count :%d\n", pages->_count);
	page_cache_get(pages);
	printk(KERN_INFO "after page_cache_get pages->_count :%d\n", pages->_count);
#if 0
#define page_cache_release(page)	put_page(page)
void put_page(struct page *page)
#endif
	page_cache_release(pages);
	printk(KERN_INFO "after page_cache_release pages->_count :%d\n", pages->_count);
finish:
	return ret;
}
