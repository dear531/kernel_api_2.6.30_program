...
static int __init set_irq_chip_data_init(void)
{
	int ret;
	int data;
	printk(KERN_INFO "set_irq_chip_data_init\n");
#if 0
...
#endif
	ret = set_irq_chip_data(irq, &data);
	printk(KERN_INFO "set_irq_chip_data result :%d\n", ret);
	return 0;
}

static void __exit set_irq_chip_data_exit(void)
{
	printk(KERN_INFO "set_irq_chip_data_exit\n");
	free_irq(irq, NULL);
}
